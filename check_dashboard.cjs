...
const elements={};const el=id=>elements[id]??={value:['winner-filter','status-filter','tier-filter','operator-filter'].includes(id)?'all':'',innerHTML:'',textContent:'',getContext:()=>({}),appendChild(){},addEventListener(){}};
const context=vm.createContext({console,window:{},document:{getElementById:el,querySelectorAll:()=>[],createElement:()=>({addEventListener(){}}),addEventListener(){}},Chart:class{constructor(c,o){this.data=o.data}destroy(){}}});
for(const path of ['data.js','dashboard.js'])vm.runInContext(fs.readFileSync(path,'utf8'),context);
